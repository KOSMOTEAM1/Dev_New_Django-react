    ...
    for i in range(start_id, finish_id):
        ...
        try:
            ...
            if imdb_id is not None:
                url = 'https://www.imdb.com/title/'+str(imdb_id[0][0])+'/ratings/?ref_=tt_ov_rt'
                r = requests.get(url)
                soup = BeautifulSoup(r.text, "html.parser")
                rimdbscore = soup.select_one("#main > section > div > div.subpage_title_block > div > div.ipl-rating-widget > div.ipl-rating-star > span.ipl-rating-star__rating")
                data = []
